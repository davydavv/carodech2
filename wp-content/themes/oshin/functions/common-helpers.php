...
if (!function_exists('be_gallery_youtube')) {
	function be_gallery_youtube( $url ) {
		...
		if( ! empty( $url ) ) {
			...
			return '<iframe class="be-youtube-video gallery" src="https://www.youtube.com/embed/'.$video_id.'?wmode=transparent" style="border: none;"></iframe>';		
		} else {
			return '';
		}
	}
...
if (!function_exists('be_gallery_vimeo')) {
	function be_gallery_vimeo( $url ) {
		...
		if( ! empty( $url ) ) {
			...
			return '<iframe class="be-vimeo-video gallery" src="https://player.vimeo.com/video/'.$video_id.'?api=1" id="be-vimeo-'.$video_id.'" frameborder=0 width="500" height="281" style="border: none;" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>';
		} else {
			return '';
		}
	}
			...
			case 'instagram':
				...
				if($transient_media && isset($transient_media) && !empty($transient_media)) {
					...
					if (isset($be_themes_data['instagram_access_token']) && !empty($be_themes_data['instagram_access_token'] ) ){
						...
						delete_transient( $transient_var );
						$return['error'] = '<div class="be-notification error">'.__('Instagram Error : Access Token is not entered under OSHINE OPTIONS > GLOBAL SITE LAYOUT AND SETTINGS. Access Token for your account can be generated from http://instagram.pixelunion.net/', 'be-themes').'</div>';
						return $return;
					}					
				}
